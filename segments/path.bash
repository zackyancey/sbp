...
if [[ $settings_path_disable_sep -eq 0 && "${#wdir_array[@]}" -gt 1 ]]; then
  segment_seperator=$(segment "$settings_path_color_sep" "$settings_path_color_bg" "$settings_char_path")
  for i in "${!wdir_array[@]}"; do
    dir=${wdir_array["$i"]}
    segment_value=$(segment "$settings_path_color_fg" "$settings_path_color_bg" " $dir ")
  ...
  path_value=" $wdir"
  segment "$settings_path_color_fg" "$settings_path_color_bg" "${path_color}${path_value} "
fi
